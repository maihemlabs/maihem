...
class Simulator:
    ...
    def conversation(
        ...
        config = cls._load_config(
            config_path, target_agent_identifier, maihem_agent_identifier
        )
    ...
    def _load_config(
        ...
        try:
            ...
            assert (
                "initiating_agent" in config
            ), "'initiating_agent' key not found in maihem_agent"
            assert (
            ...
            assert (
                maihem_agent_identifier in config["maihem_agent"].keys()
            ), f"Maihem agent '{maihem_agent_identifier}' not found in config file"

            for identifier, target_agent in config["target_agent"].items():
                assert (
                    "role" in target_agent
                ), f"'role' key not found in target agent {identifier}"
                assert (
                    "industry" in target_agent
                ), f"'industry' key not found in target agent {identifier}"
                assert (
                    "description" in target_agent
                ), f"'description' key not found in target agent {identifier}"
            ...
            for identifier, maihem_agent in config["maihem_agent"].items():
                ...
                maihem_agent["conversation_turns_max"] = maihem_agent.get(
                    "conversation_turns_max"
                )
