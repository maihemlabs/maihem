...
from maihem import Maihem, MaihemAsync
from maihem.agents import AgentTarget, AgentTargetAsync, AgentMaihem
...
def chat_function(
    ...
    if agent_maihem_end_code is None: # If no end code is provided from maihem agent
        agent_target_message = f"({datetime.now()}) Your target agent message when no end code is provided"
        ...
        agent_target = f"({datetime.now()}) Your target agent message when end code is provided"
        end = "<END_CODE>" # True if the conversation should end, False if it should continue
...
async def async_chat_function(
    conversation_id: str,
    agent_maihem_msg: str, 
    agent_maihem_end_code: Optional[str] 
) -> Tuple[str, bool]:
    """
    ...
    if agent_maihem_end_code is None: # If no end code is provided from maihem agent
        response_message = f"({datetime.now()}) Your target agent message when no end code is provided"
        end = False # True if the conversation should end, False if it should continue
    else:  # If end code is provided from maihem agent
        response_message = f"({datetime.now()}) Your target agent message when end code is provided"
...
test_run_results: TestRunResults = m.get_test_run_results(   # AgentTarget to be tested with callable function
